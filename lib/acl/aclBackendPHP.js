...
class aclBackendPHP {
	...
	constructor() {
		console.log(process.env.ACLBACKENDTURL)
		this.apiCall = createAPI({
			baseURL: process.env.ACLBACKENDTURL,
			headers: {
	...
	async removePrivileges(resourceUUID, entityUUID, privilege) {
		let permission = new Permission({ "privilege": privilege, "entities": [entityUUID] })
		return await this.apiCall.delete('permission/' + resourceUUID, permission).then(rs => {
			return new Entity(rs.data)
		})
	}
	...
	async listPrivileges(entityUUID, resourceUUID) {
		return await this.apiCall.get('listPrivileges/' + entityUUID + '/' + resourceUUID).then(rs => {
	...
	async findResources(entityUUID, resourceUUID, majorType, privilege = '', minorType = 0) {
		return await this.apiCall.get('findResources/' + entityUUID + '/' + resourceUUID + '/' + majorType  + '/' + privilege).then(rs => {
			console.log('findResources/' + entityUUID + '/' + resourceUUID + '/' + majorType  + '/' + privilege, rs.ok)
			return rs.data
		})
	}
