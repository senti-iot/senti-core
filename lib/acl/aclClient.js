const aclBackendPHP = require('./aclBackendPHP')
...
class aclClient {
	...
	constructor() {
		this.backend = new aclBackendPHP()
	}
	...
	async removePrivileges(resourceUUID, entityUUID, privilege) {
		return await this.backend.removePrivileges(resourceUUID, entityUUID, privilege)
	}
	...
	async listPrivileges(entityUUID, resourceUUID) {
		let result = await this.backend.listPrivileges(entityUUID, resourceUUID)
		return result
	}
