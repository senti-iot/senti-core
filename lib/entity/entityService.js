...
class entityService {
	...
	async getDbOrganisationsByUUID(uuids = false) {
		...
		let orgClause = (uuids.length > 0) ? ' AND O.uuid IN (?' + ",?".repeat(uuids.length - 1) + ') ' : ''
		let select = `SELECT id, uuid, uuname, name, nickname, address, zip, city, country, website, aux, internal, parentOrgId, deleted, created, modified
			FROM organisation O 
			WHERE O.deleted = 0 ${orgClause};`
		let rs = await mysqlConn.query(select, uuids)
		if (rs[0].length === 0) {
	...
	async createOrganisation(org = false) {
		...
		let dbO = new DbOrganisation(org)
		dbO.uuid = uuidv4()
		let organisationSQL = `INSERT INTO organisation(uuid, uuname, name, nickname, address, zip, city, country, website, aux, internal, parentOrgId, deleted, created, modified) 
			VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, NOW(), NOW());`

		let sql = await mysqlConn.format(organisationSQL, [dbO.uuid, dbO.uuname, dbO.name, dbO.nickname, dbO.address, dbO.zip, dbO.city, dbO.country, dbO.website, 
			(dbO.aux !== null) ? JSON.stringify(dbO.aux) : dbO.aux, 
			(dbO.internal !== null) ? JSON.stringify(dbO.internal) : dbO.internal, 
			dbO.parentOrgId, dbO.deleted])
		console.log(sql)
		
		let rs = await mysqlConn.query(organisationSQL, [dbO.uuid, dbO.uuname, dbO.name, dbO.nickname, dbO.address, dbO.zip, dbO.city, dbO.country, dbO.website, 
			(dbO.aux !== null) ? JSON.stringify(dbO.aux) : dbO.aux, 
			(dbO.internal !== null) ? JSON.stringify(dbO.internal) : dbO.internal, 
			dbO.parentOrgId, dbO.deleted])
		if (rs[0].affectedRows === 1) {
	...
	async updateOrganisation(org = false) {
		...
		let organisationSQL = `UPDATE organisation
			SET uuname = ?, 
				...
				modified = NOW()
			WHERE id = ?;`
		let rs = await mysqlConn.query(organisationSQL, [dbO.uuname, dbO.name, dbO.nickname, dbO.address, dbO.zip, dbO.city, dbO.country, dbO.website, 
			(dbO.aux !== null) ? JSON.stringify(dbO.aux) : dbO.aux, 
			(dbO.internal !== null) ? JSON.stringify(dbO.internal) : dbO.internal, 
	...
	getUUName(name) {
		return name.toLowerCase().replace(' ', '-').replace('---', '-').replace('--', '-').replace('/', '') + '-' + this.shortHashGen(uuidv4())
	}
	getNickname(name) {
