...
class entityService {
	...
	async createUser(user = false) {
		...
		if (rs[0].affectedRows === 1) {
			return await this.getUserById(rs[0].insertId)
		}
		return false
	}
	...
	async setUserPassword(credentials = false) {
		...
		let update = `UPDATE user
			SET password = ?, 
				modified = NOW()
			WHERE id = ?;`
		let rs = await mysqlConn.query(update, [authClient.getPWHash(dbO.newPassword), dbO.id])
		if (rs[0].affectedRows === 1) {
			return true
		}
		return false
